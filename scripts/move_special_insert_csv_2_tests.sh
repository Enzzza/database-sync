...
start() {
    ...
    for dir in */; do
        ...
        cp "$csv_file" "$TEST_DEST_DIR/special_insert/"
        csv_file_name=$(basename "$csv_file")
        chmod "$PERMISSIONS" "$TEST_DEST_DIR/special_insert/$csv_file_name"
        chgrp "$GROUP" "$TEST_DEST_DIR/special_insert/$csv_file_name"
